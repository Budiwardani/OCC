...
Sudah diperbaiki: scope company/assignment pada query complaint utama, validasi assignee lintas perusahaan, endpoint dan UI respons, audit log untuk update/respons, pencatatan SLA awal, migration runner idempoten, bootstrap schema Docker, token tracking dan upload publik, rate limit publik/AI, batas upload 5 MB, data terbaru publik yang tidak lagi memuat deskripsi, dashboard activity feed, route Surat Kuasa duplikat, konsistensi kredensial Superadmin, alias admin `/admin`, migration phone eksplisit, integrasi service OpenWA configurable dengan pengiriman non-blocking, serta Worker Cloudflare opsional untuk proxy WhatsApp.

Masih residual: URL publik backend Express dan TLS/CORS production, provider email nyata, image OpenWA/QR pairing/API key di setiap deployment, antivirus/content scanning upload, download file dengan authorization penuh, CAPTCHA, job SLA breach otomatis, test suite bisnis lengkap, dan hardening production image/dependency.
