...
const ChatWidget = () => {
    ...
    const handleSearch = async (e) => {
        ...
        if (!trimmedQuery) {
            setSearchError("Masukkan nomor HP, nomor tiket, atau nama.");
            return;
        }
        setSearchError("");
        setSearching(true);
        ...
        try {
            ...
            try {
                ...
                console.warn("AI search direct API fallback:", aiErr);
                // Fallback to public tracking or dashboard complaints
                if (isAdmin) {
            ...
            setSearchError(err.response?.data?.error || "Gagal melakukan pencarian. Pastikan kata kunci sudah benar.");
        } finally {
            setSearching(false);
        }
    };
    ...
    const sendMessage = async (textToSend) => {
        ...
        try {
            ...
            setMessages((c) => [...c, { role: "bot", content: "Halo! Saya Virtual Assistant OCC. Silakan gunakan tab 'Cari Tiket & Status' untuk mengecek laporan tiket Anda berdasarkan nomor HP." }]);
        } finally {
    ...
    // ─── Minimized FAB (Floating Button) ────────────────────────
    if (!isOpen) {
        return (
            <button
                onClick={() => setIsOpen(true)}
                className="fixed bottom-5 right-5 z-50 flex items-center gap-2.5 rounded-full bg-primary-600 hover:bg-primary-700 px-4 py-3 text-white shadow-xl hover:shadow-2xl transition-all duration-200 transform hover:-translate-y-0.5"
                title="Buka Virtual Assistant OCC"
        ...
        <div className="fixed bottom-5 right-5 z-50 flex flex-col w-[360px] sm:w-[420px] max-h-[600px] rounded-2xl border border-secondary-200 bg-white shadow-2xl overflow-hidden animate-fadeIn">
            ...
            <div className="flex border-b border-secondary-200 bg-white flex-shrink-0">
                <button
                    ...
                    className={`flex-1 py-2.5 text-xs font-semibold transition-colors flex items-center justify-center gap-1.5 ${activeTab === TAB_SEARCH
                        ? "border-b-2 border-primary-600 text-primary-600 bg-primary-50/40"
                        : "text-secondary-500 hover:text-secondary-700"}`}
                >
                    <span>🔍</span> Cari Tiket & Status
                </button>
                <button
                    onClick={() => setActiveTab(TAB_CHAT)}
                    className={`flex-1 py-2.5 text-xs font-semibold transition-colors flex items-center justify-center gap-1.5 ${activeTab === TAB_CHAT
                        ? "border-b-2 border-primary-600 text-primary-600 bg-primary-50/40"
                        : "text-secondary-500 hover:text-secondary-700"}`}
                >
                    <span>💬</span> Tanya VA
                ...
                <div className="flex flex-col flex-1 overflow-hidden">
                        ...
                        <form onSubmit={handleSearch} className="space-y-2">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="flex-1 rounded-xl border border-secondary-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 bg-white placeholder-secondary-400"
                                />
                                <button
                                    type="submit"
                                    disabled={searching}
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-2.5 bg-secondary-50">
                        {searchResults === null && !searching && (
                            <div className="flex flex-col items-center justify-center h-full text-center text-secondary-400 py-8 px-2">
                                <div className="w-12 h-12 rounded-full bg-primary-100 flex items-center justify-center text-primary-600 mb-2.5">
                                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 21l-4.35-4.35M11 19a8 8 0 100-16 8 8 0 000 16z" />
                                    </svg>
                                </div>
                                <p className="text-sm font-semibold text-secondary-700">Cari Tiket & Status Keluhan</p>
                                <p className="text-xs text-secondary-400 mt-1 max-w-[260px]">
                                    Masukkan nomor HP pelapor untuk melihat daftar tiket, token publik, dan status pengerjaannya.
                                </p>
                            </div>
                        )}
                        ...
                        {searchResults !== null && searchResults.total === 0 && (
                            <div className="flex flex-col items-center justify-center h-full text-center py-8 px-2">
                                <div className="w-10 h-10 rounded-full bg-secondary-100 flex items-center justify-center text-secondary-400 mb-2">
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
                                <p className="text-sm font-semibold text-secondary-700">Tidak ada laporan ditemukan</p>
                                <p className="text-xs text-secondary-400 mt-1">Pastikan nomor HP atau nomor tiket sudah sesuai dengan yang dilaporkan.</p>
                            </div>
                        )}
                            ...
                            <div key={c.ticket_code || c.id} className="rounded-xl border border-secondary-200 bg-white p-3.5 shadow-sm hover:shadow-md transition-all">
                                ...
                                <div className="flex items-start justify-between gap-2 mb-1.5">
                                    <div className="min-w-0 flex-1">
                                        <p className="text-xs font-bold text-primary-700 font-mono break-all select-all">{c.ticket_code}</p>
                                        <p className="text-xs font-semibold text-secondary-800 truncate">{c.customer_name}</p>
                                    </div>
                                    <span className={`flex-shrink-0 px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${STATUS_COLORS[c.status] || "bg-gray-100 text-gray-800 border-gray-200"}`}>
                                ...
                                <div className="pt-2 border-t border-secondary-100 flex items-center justify-between gap-2">
                                    ...
                                    {isAdmin && c.id ? (
                                        ...
                                        <button
                                            onClick={() => {
                                                setIsOpen(false);
                                                // If on portal, trigger tracking query
                                                window.location.href = `/?tab=tracking&ticket=${encodeURIComponent(c.ticket_code)}&token=${encodeURIComponent(c.public_token || '')}`;
                                            }}
                                            className="text-[11px] font-semibold text-primary-600 hover:text-primary-800 hover:underline flex items-center gap-0.5"
                                        >
                                            Lacak Status →
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-secondary-50 text-sm">
                        {messages.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full text-center py-6 px-3">
                                ...
                                <p className="text-xs text-secondary-500 mt-1 mb-3">Pilih pertanyaan cepat atau ketik pertanyaan Anda di bawah:</p>

                                {/* Quick Suggestions */}
                                <div className="flex flex-col gap-1.5 w-full text-left">
                                    <button
                                        onClick={() => handleQuickAction("Bagaimana cara membuat laporan keluhan baru?")}
                                        className="text-xs text-secondary-700 bg-white hover:bg-primary-50 hover:text-primary-700 border border-secondary-200 rounded-xl px-3 py-2 transition-colors text-left"
                                    >
                                        📝 Cara membuat laporan keluhan
                                    </button>
                                    <button
                                        onClick={() => handleQuickAction("Bagaimana cara mengecek status tiket pengaduan?")}
                                        className="text-xs text-secondary-700 bg-white hover:bg-primary-50 hover:text-primary-700 border border-secondary-200 rounded-xl px-3 py-2 transition-colors text-left"
                                    >
                                        🔍 Cara cek status tiket & token
                                    </button>
                                    <button
                                        onClick={() => handleQuickAction("Bagaimana cara upload dokumen Surat Kuasa?")}
                                        className="text-xs text-secondary-700 bg-white hover:bg-primary-50 hover:text-primary-700 border border-secondary-200 rounded-xl px-3 py-2 transition-colors text-left"
                                    >
                                        📄 Panduan upload Surat Kuasa
                                    </button>
                                </div>
                            </div>
                        ) : (
                                ...
                                <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                                    <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm ${
                                        m.role === "user"
                                            ...
                                            : "bg-white border border-secondary-200 text-secondary-800 rounded-bl-none shadow-sm whitespace-pre-line"
                                    }`}>
                                        {m.content}
                                    </div>
                                </div>
