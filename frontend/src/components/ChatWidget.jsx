...
const ChatWidget = () => {
    ...
    const handleSearch = async (e) => {
        ...
        if (trimmedPhone.length < 5) {
            setSearchError("Nomor HP minimal 5 digit.");
            return;
        }
        setSearchError("");
        setSearching(true);
        ...
        try {
            const params = { phone: trimmedPhone };
            if (statusFilter !== "ALL") params.status = statusFilter;
            if (categoryFilter !== "ALL") params.category = categoryFilter;
            const { data } = await api.get("/ai/search", { params });
            setSearchResults(data);
        } catch (err) {
            setSearchError(err.response?.data?.error || "Gagal mencari data.");
        } finally {
            setSearching(false);
        }
    };
    ...
    // ─── Expanded widget ──────────────────────────────────────
    return (
        <div className="fixed bottom-5 right-5 z-50 flex flex-col w-[360px] sm:w-[420px] max-h-[600px] rounded-2xl border border-secondary-200 bg-white shadow-2xl overflow-hidden">
                        ...
                        <form onSubmit={handleSearch} className="space-y-2">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    placeholder="Masukkan No HP (cth: 081234567890)"
                                    className="flex-1 rounded-xl border border-secondary-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
                                />
                                <button
                                    type="submit"
                                    disabled={searching}
                                    className="rounded-xl bg-primary-600 hover:bg-primary-700 disabled:opacity-50 px-3 py-2 text-white text-sm transition-colors"
                                >
                                    {searching ? (
                            ...
                            <div className="flex gap-2">
                                <select
                                    ...
                                    className="flex-1 rounded-xl border border-secondary-300 px-2 py-1.5 text-xs focus:border-primary-500 focus:outline-none bg-white"
                                >
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-2">
                        {searchResults === null && !searching && (
                            <div className="flex flex-col items-center justify-center h-full text-center text-secondary-400 py-8">
                                ...
                                <p className="text-sm font-medium text-secondary-500">Cari keluhan berdasarkan No HP</p>
                                <p className="text-xs text-secondary-400 mt-1">Masukkan nomor HP pelanggan untuk melihat semua tiket dan status keluhannya</p>
                            </div>
                        )}
                        ...
                        {searchResults !== null && searchResults.total === 0 && (
                            <div className="flex flex-col items-center justify-center h-full text-center py-8">
                                ...
                                <p className="text-sm font-medium text-secondary-600">Tidak ada keluhan ditemukan</p>
                                <p className="text-xs text-secondary-400 mt-1">Coba nomor lain atau ubah filter</p>
                            </div>
                        )}

                        {searchResults !== null && searchResults.results.map((c) => (
                            <div key={c.ticket_code} className="rounded-xl border border-secondary-200 bg-white p-3 shadow-sm hover:shadow-md transition-shadow">
                                {/* Header row */}
                                <div className="flex items-start justify-between gap-2 mb-1.5">
                                    <div>
                                        <p className="text-xs font-bold text-secondary-800 font-mono">{c.ticket_code}</p>
                                        <p className="text-xs text-secondary-500">{c.customer_name}</p>
                                    </div>
                                    <span className={`flex-shrink-0 px-2 py-0.5 rounded-full text-[10px] font-semibold ${STATUS_COLORS[c.status] || "bg-gray-100 text-gray-800"}`}>
                                        {STATUS_LABELS[c.status] || c.status}
                                    </span>
                                </div>

                                {/* Subject */}
                                <p className="text-xs text-secondary-700 mb-1.5 line-clamp-2">{c.subject}</p>
                                ...
                                <div className="mt-2 pt-2 border-t border-secondary-100">
                                    <p className="text-[10px] text-secondary-400">
                                        Token: <span className="font-mono text-secondary-600 select-all">{c.public_token}</span>
                                    </p>
                                </div>
                            </div>
                        ))}
                        ...
                        {searchResults !== null && searchResults.total > 0 && (
                            <p className="text-center text-[10px] text-secondary-400 pt-1">
                                Menampilkan {searchResults.total} hasil
                            </p>
                        )}
                    </div>
                </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-secondary-50 text-sm">
                        {messages.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full text-center py-6 px-3">
                                ...
                                <p className="text-xs text-secondary-500 mt-1">Ada yang bisa saya bantu?</p>
                            </div>
                        ) : (
                            messages.map((m, i) => (
                    ...
                    <div className="border-t border-secondary-200 p-3 bg-white flex-shrink-0">
                        <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-2">
                            ...
                            <button
                                ...
                                className="rounded-xl bg-primary-600 hover:bg-primary-700 disabled:opacity-50 px-3.5 py-2 text-white transition-colors flex items-center"
                            >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
