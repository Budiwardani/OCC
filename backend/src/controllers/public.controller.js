...
export const getLatestComplaints = async (req, res) => {
    try {
        const query = `
            SELECT ticket_code, subject, description, status, city, created_at 
            FROM complaints 
...
export const submitComplaint = async (req, res) => {
    ...
    try {
        ...
        if (files.length > 0) {
            const attachmentValues = files.map(file => `(${newComplaint.id}, '${file.path.replace(/\\/g, '/')}')`).join(',');
            await db.query(`INSERT INTO complaint_attachments (complaint_id, file_path) VALUES ${attachmentValues}`);
        }
...
export const trackComplaint = async (req, res) => {
    const { ticket, email } = req.query; // Changed from token to email based on new requirement
    ...
    try {
        ...
        const { rows } = await db.query(
         ...
         FROM complaints 
         WHERE ticket_code=$1 AND customer_email=$2`,
            [ticket, email]
        );
